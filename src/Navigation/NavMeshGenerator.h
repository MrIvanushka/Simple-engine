...
public:
    static NavMeshSurface* generate(std::vector<glm::vec3> vertices, std::vector<unsigned int> indices, std::shared_ptr<Engine::Transform> center){
        ...
        {
            checked[i] = false;
            float dist = glm::distance(zero, vertices[i]);
            ...
            points[i] = center->rotation() * (center->position() + vertices[i] * center->scale());
        }
